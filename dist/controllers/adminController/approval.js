...
import { sendApprovalEmail, sendRejectionEmail, } from "../../emailService/approvalEmail/userApproval.js";
export const getAllPendingSeeker = async (_req, res) => {
    try {
...
export const getAllPendingEmployer = async (_req, res) => {
    try {
...
export const approveEmployerById = async (req, res) => {
    try {
        const { id } = req.params;
...
export const approveJobSeekerById = async (req, res) => {
    try {
        const { id } = req.params;
        const user = await User.findById(id);
...
export const rejectJobSeekerById = async (req, res) => {
    try {
        const { id } = req.params;
        const user = await User.findById(id);
        if (!user || user.role !== "job_seeker") {
        ...
        if (!emailSent) {
            console.warn("Rejection email failed to send, but proceeding with deletion.");
        }
...
export const rejectEmployerById = async (req, res) => {
    try {
        const { id } = req.params;
        const user = await User.findById(id);
        if (!user || user.role !== "employer") {
